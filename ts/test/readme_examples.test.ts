...
//     (\`new SwissFederalRailwaysSbbSDK(...)\` / \`SwissFederalRailwaysSbbSDK.test(...)\` become
//     \`SwissFederalRailwaysSbbSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SwissFederalRailwaysSbbSDK'
...
const TEST_SEED = {"entity":{"export":{"test01":{"id":"test01"}},"record":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SwissFederalRailwaysSbbSDK(...)\` and any
// \`SwissFederalRailwaysSbbSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SwissFederalRailwaysSbbSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SwissFederalRailwaysSbbSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
