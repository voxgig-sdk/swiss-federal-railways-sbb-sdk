...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*SwissFederalRailwaysSbbSDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *SwissFederalRailwaysSbbError {
	return NewSwissFederalRailwaysSbbError(code, msg, ctx)
}
