...
class SwissFederalRailwaysSbbMakeRequest
{
    public static function call(SwissFederalRailwaysSbbContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new SwissFederalRailwaysSbbResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new SwissFederalRailwaysSbbResponse($fetched);
        } else {
