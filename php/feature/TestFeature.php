...
// SwissFederalRailwaysSbb SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class SwissFederalRailwaysSbbTestFeature extends SwissFederalRailwaysSbbBaseFeature
{
    ...
    public function init(SwissFederalRailwaysSbbContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (SwissFederalRailwaysSbbContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = SwissFederalRailwaysSbbParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (SwissFederalRailwaysSbbContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(SwissFederalRailwaysSbbContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = SwissFederalRailwaysSbbParam::call($ctx, $k);
