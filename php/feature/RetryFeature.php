...
// SwissFederalRailwaysSbb SDK retry feature
...
class SwissFederalRailwaysSbbRetryFeature extends SwissFederalRailwaysSbbBaseFeature
{
    ...
    public function init(SwissFederalRailwaysSbbContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SwissFederalRailwaysSbbContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SwissFederalRailwaysSbbContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SwissFederalRailwaysSbbContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
