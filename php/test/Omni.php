...
//    only - a typed SwissFederalRailwaysSbbContext there would make every ctx
...
final class SwissFederalRailwaysSbbOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SwissFederalRailwaysSbbContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new SwissFederalRailwaysSbbSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new SwissFederalRailwaysSbbResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new SwissFederalRailwaysSbbError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new SwissFederalRailwaysSbbResponse($clean['response']);
    ...
    public static function err_from_map($m): ?SwissFederalRailwaysSbbError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SwissFederalRailwaysSbbError(is_string($code) ? $code : '', $msg);
    }
