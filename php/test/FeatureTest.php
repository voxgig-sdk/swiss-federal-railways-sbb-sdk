...
// SwissFederalRailwaysSbb SDK feature test
//
...
class FtCtrl extends SwissFederalRailwaysSbbControl
{
...
class FtClient
{
    ...
    private SwissFederalRailwaysSbbUtility $utility;

    public function __construct(SwissFederalRailwaysSbbUtility $utility, array $options)
    {
    ...
    public function get_utility(): SwissFederalRailwaysSbbUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SwissFederalRailwaysSbbContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SwissFederalRailwaysSbbContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new SwissFederalRailwaysSbbUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (SwissFederalRailwaysSbbContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SwissFederalRailwaysSbbConfig::make_feature($name);
    ...
    private static function buildUrl(SwissFederalRailwaysSbbSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SwissFederalRailwaysSbbError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SwissFederalRailwaysSbbResponse($fetched);
        }
    ...
    private function fail(SwissFederalRailwaysSbbContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SwissFederalRailwaysSbbContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
