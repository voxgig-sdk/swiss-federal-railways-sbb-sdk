// Command SwissFederalRailwaysSbb-cli is a CLI + REPL for the SwissFederalRailwaysSbb SDK,
...
const prompt = "swiss-federal-railways-sbb"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "export record"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SWISS_FEDERAL_RAILWAYS_SBB_APIKEY (api key), SWISS_FEDERAL_RAILWAYS_SBB_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SWISS_FEDERAL_RAILWAYS_SBB_APIKEY carries the API key and
	// SWISS_FEDERAL_RAILWAYS_SBB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SWISS_FEDERAL_RAILWAYS_SBB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SWISS_FEDERAL_RAILWAYS_SBB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSwissFederalRailwaysSbbSDK(opts)
