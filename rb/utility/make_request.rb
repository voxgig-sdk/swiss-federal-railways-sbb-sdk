...
module SwissFederalRailwaysSbbUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SwissFederalRailwaysSbbResponse.new({})
    ...
    if fetch_err
      ...
      response = SwissFederalRailwaysSbbResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SwissFederalRailwaysSbbResponse.new(fetched)
    else
