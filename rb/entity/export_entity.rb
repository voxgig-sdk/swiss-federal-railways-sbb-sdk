...
class ExportEntity
  ...
  def data_set(args)
    if args
      @_data = SwissFederalRailwaysSbbHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = SwissFederalRailwaysSbbHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Export, Hash] the loaded Export; raises SwissFederalRailwaysSbbError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SwissFederalRailwaysSbbHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ExportListMatch, Hash, nil] match filter (any subset of
  #   Export fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Export>, Array] the matching Export items; raises SwissFederalRailwaysSbbError on failure
