# SwissFederalRailwaysSbb Ruby SDK

The Ruby SDK for the SwissFederalRailwaysSbb API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Export(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.SwissFederalRailwaysSbb(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── SwissFederalRailwaysSbb_sdk.rb       -- Main SDK module
...
The main module (`SwissFederalRailwaysSbb_sdk`) exports the SDK class
