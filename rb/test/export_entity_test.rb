...
class ExportEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = SwissFederalRailwaysSbbSDK.test(seed, nil)
    seen = base.Export(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = SwissFederalRailwaysSbbConfig.make_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = SwissFederalRailwaysSbbSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.Export(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SWISS_FEDERAL_RAILWAYS_SBB_TEST_EXPORT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def export_basic_setup(extra)
  ...
  entid_env_raw = ENV["SWISS_FEDERAL_RAILWAYS_SBB_TEST_EXPORT_ENTID"]
  ...
  if env["SWISS_FEDERAL_RAILWAYS_SBB_TEST_LIVE"] == "TRUE"
    ...
    client = SwissFederalRailwaysSbbSDK.new(Helpers.to_map(merged_opts))
  end
