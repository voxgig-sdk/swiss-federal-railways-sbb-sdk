# swiss-federal-railways-sbb-mcp

[MCP](https://modelcontextprotocol.io) server exposing the SwissFederalRailwaysSbb SDK as
two agent tools — `swiss-federal-railways-sbb_list` and `swiss-federal-railways-sbb_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/swiss-federal-railways-sbb-mcp)
make build
...
claude mcp add --scope user swiss-federal-railways-sbb \
  -- /absolute/path/to/swiss-federal-railways-sbb-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./swiss-federal-railways-sbb-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// swiss-federal-railways-sbb_list: first page of records
{ "entity": "export" }
{ "entity": "export", "query": { } }

// swiss-federal-railways-sbb_load: one record by id
{ "entity": "export", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/swiss-federal-railways-sbb-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user swiss-federal-railways-sbb \
     -- "$PWD"/dist/*/swiss-federal-railways-sbb-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `swiss-federal-railways-sbb_list` and `swiss-federal-railways-sbb_load` tools now appear
   in new sessions. Ask the agent to *"list export using swiss-federal-railways-sbb"*
   and it calls `swiss-federal-railways-sbb_list` with `{"entity":"export"}`.
...
### Call the `swiss-federal-railways-sbb_list` tool
...
### Call the `swiss-federal-railways-sbb_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `swiss-federal-railways-sbb_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `swiss-federal-railways-sbb_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./swiss-federal-railways-sbb-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"swiss-federal-railways-sbb_load","arguments":{"entity":"export","query":{"id":1}}}}'
```
