...
import (
	...
	omni "github.com/voxgig-sdk/swiss-federal-railways-sbb-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SwissFederalRailwaysSbbError); is {
				out := map[string]any{
					"name":    "SwissFederalRailwaysSbbError",
...
func omniCtx(arg any, client *sdk.SwissFederalRailwaysSbbSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SwissFederalRailwaysSbbSDK); is {
			client = live
