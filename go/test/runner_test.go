...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("SWISS_FEDERAL_RAILWAYS_SBB_TEST_LIVE") == "TRUE" ||
		os.Getenv("SWISS_FEDERAL_RAILWAYS_SBB_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("SWISS_FEDERAL_RAILWAYS_SBB_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.SwissFederalRailwaysSbbSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.SwissFederalRailwaysSbbSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.SwissFederalRailwaysSbbError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.SwissFederalRailwaysSbbSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.SwissFederalRailwaysSbbError{Msg: msg, Code: code}
}
