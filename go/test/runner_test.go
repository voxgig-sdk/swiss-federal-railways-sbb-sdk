...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("SWISSFEDERALRAILWAYSSBB_TEST_LIVE") == "TRUE" ||
		os.Getenv("SWISSFEDERALRAILWAYSSBB_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("SWISSFEDERALRAILWAYSSBB_TEST_EXPLAIN"); explain != "" {
