# SwissFederalRailwaysSbb Lua SDK

The Lua SDK for the SwissFederalRailwaysSbb API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Export(nil):list(nil, nil)
...
```lua
local result, err = client:Export(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:SwissFederalRailwaysSbb(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── swiss-federal-railways-sbb_sdk.lua    -- Main SDK module
...
The main module (`swiss-federal-railways-sbb_sdk`) exports the SDK constructor
