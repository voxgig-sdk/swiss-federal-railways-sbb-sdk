# SwissFederalRailwaysSbb SDK test runner
...
class SwissFederalRailwaysSbbTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SwissFederalRailwaysSbbTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SwissFederalRailwaysSbbTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SwissFederalRailwaysSbbTestRunner.getenv("SWISS_FEDERAL_RAILWAYS_SBB_TEST_LIVE")
        override = SwissFederalRailwaysSbbTestRunner.getenv("SWISS_FEDERAL_RAILWAYS_SBB_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SwissFederalRailwaysSbbTestRunner.getenv(key)
        ...
        explain = SwissFederalRailwaysSbbTestRunner.getenv("SWISS_FEDERAL_RAILWAYS_SBB_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if SwissFederalRailwaysSbbTestRunner._test_control is not None:
            return SwissFederalRailwaysSbbTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SwissFederalRailwaysSbbTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SwissFederalRailwaysSbbTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SwissFederalRailwaysSbbTestRunner.load_test_control()
...
def load_env_local():
    SwissFederalRailwaysSbbTestRunner.load_env_local()


def env_override(m):
    return SwissFederalRailwaysSbbTestRunner.env_override(m)


def entity_data(v):
    return SwissFederalRailwaysSbbTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SwissFederalRailwaysSbbTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SwissFederalRailwaysSbbTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SwissFederalRailwaysSbbTestRunner.load_test_control()


def live_client_options():
    return SwissFederalRailwaysSbbTestRunner.live_client_options()


def live_delay_ms():
    return SwissFederalRailwaysSbbTestRunner.live_delay_ms()
